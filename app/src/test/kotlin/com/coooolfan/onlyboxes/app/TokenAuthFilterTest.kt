...
class TokenAuthFilterTest {
    ...
    @Test
    fun successRequestBindsAndClearsTokenContext() {
        val filter = TokenAuthFilter(
            headerName = "X-Onlyboxes-Token",
            allowedTokens = setOf("abc123"),
            protectedEndpoint = "/mcp",
        ...
        val response = MockHttpServletResponse()
        val chain = TrackingFilterChain {
            assertEquals("abc123", AuthTokenContextHolder.currentToken())
        }

        filter.doFilter(request, response, chain)

        assertTrue(chain.invoked)
        assertNull(AuthTokenContextHolder.currentToken())
    }

    @Test
    fun failedRequestDoesNotBindTokenContext() {
        val filter = TokenAuthFilter(
            headerName = "X-Onlyboxes-Token",
            allowedTokens = setOf("abc123"),
            protectedEndpoint = "/mcp",
        ...
        val request = MockHttpServletRequest("POST", "/mcp").apply {
            addHeader("X-Onlyboxes-Token", "bad-token")
        }
        val response = MockHttpServletResponse()
        val chain = TrackingFilterChain()
    ...
    private class TrackingFilterChain(
        private val onFilter: (() -> Unit)? = null,
    ) : FilterChain {
